...
Vue.component('ckeditor2', {
 ...
 let app = new Vue({
    el: '#app',
    data:{
        titulo:"Investigación FEC",
        subtitulo: "Gestión de Revistas",
        ...
        fillobject:{ 'id':'', 'titulo':'', 'fecha':'', 'descripcion':'','nombrefile':'','urlimagen':'', 'urlrevista':'','activo':'','oldImg':'', 'oldFile':'', 'tieneimagen':'','tienerevista':''},
    ...
    methods: {
        ...
        getDatos: function (page) {
            ...
            var v3 = 0;
            var url = '/intranet/revistasre?page='+page+'&busca='+busca+'&v1='+v1+'&v2='+v2+'&v3='+v3;
        ...
        create:function () {
            ...
            data.append('tienerevista', this.tienerevista);
            data.append('urlimagen', this.urlimagen);
            data.append('urlrevista', this.urlrevista);
        ...
        borrar:function (dato) {
          swal.fire({
              title: '¿Estás seguro?',
              text: "¿Desea eliminar el contenido seleccionada? -- Nota: Este proceso no se podrá revertir",
              type: 'info',
            ...
            if (result.value) {
                var url = '/intranet/revistasre/'+dato.id;
        ...
        borrarImage:function (dato) {
          swal.fire({
            ...
            if (result.value) {
                var url = '/intranet/revistasre/deleteimg/'+dato.id;
                axios.delete(url).then(response=>{//eliminamos
        ...
        borrarFile:function (dato) {
          swal.fire({
            ...
            if (result.value) {
                var url = '/intranet/revistasre/deletefile/'+dato.id;
                axios.delete(url).then(response=>{//eliminamos
        ...
        cerrarFormE: function(){
            ...
            this.$nextTick(function () {
                this.fillobject={ 'id':'', 'titulo':'', 'descripcion':'', 'fecha':'','nombrefile':'','urlimagen':'', 'urlrevista':'','activo':'','oldImg':'', 'oldFile':'', 'tieneimagen':'','tienerevista':''};
        ...
        update: function (id) {
            ...
            var url="/intranet/revistasre/"+id;
            ...
            data.append('urlrevista', this.fillobject.urlrevista);
            data.append('activo', this.fillobject.activo);
            data.append('tieneimagen', this.fillobject.tieneimagen);
            data.append('tienerevista', this.fillobject.tienerevista);
        ...
        baja:function (dato) {
          swal.fire({
            ...
            if (result.value) {
                var url = '/intranet/revistasre/altabaja/'+dato.id+'/0';
      ...
      alta:function (dato) {
          swal.fire({
            ...
            if (result.value) {
                var url = '/intranet/revistasre/altabaja/'+dato.id+'/1';
                axios.get(url).then(response=>{//eliminamos
